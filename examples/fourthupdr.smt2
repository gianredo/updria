...
;;unsat
;;this generalisation seems fine
...
;; are there any bad models in F_1 & neg(bad) & H
(push 1)
    (assert (and (not diagram_weaken) (not hat-prop) H))
    (check-sat)
(pop 1)
;; no more models

;; what about the other generalization
(push 1)
    (assert (and (not diagram_weaken_1) (not hat-prop) H))
    (check-sat)
    ;;(get-model)
(pop 1)

;;  new diagram 
(define-fun diagram_1 () Bool 
    (exists ((i1 index) (i2 index))
    (and 
        ;; (not (= i1 i2))
        x_to_serve_equals_1
...
(define-fun diagram_1_next () Bool 
    (exists ((i1 index) (i2 index))
    (and 
        ;; (not (= i1 i2))
        x_to_serve_equals_1^1
        (not x_next_token_equals_1^1)
        (not (x_token_equals_0^1 i1))
        (not (x_token_equals_0^1 i2))
        (x_state_crit^1 i1)
...
(push 1)
    ...
    (assert (not diagram_1))
    (assert diagram_1_next)
    (check-sat)
(pop 1)

;; new generalization? so on and so on...