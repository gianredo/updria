...
def do_filter(iterable):
    ...
    for line in iterable:
        ...
        if line.startswith('Proved!'):
            ...
            try:
                _, num_initial_preds = line.split('are', 1)
            except ValueError: pass
        ...
        elif line.startswith('no new predicates found! increasing size...'):
            failed_refinements += 1

        elif line.startswith('starting refinemnt #'):
            num_ref_steps += 1

        elif line.startswith('Add new counter'):
            last_frame += 1
    ...
    t = Template('$result, $tot_time, $z3_time, $gen_time, $rec_block_time, $propagation_time, $refinement_time, $minimization_models_time, $num_z3_calls, $added_diagrams_frame, $greatest_num_literals_frame, $num_ref_steps, $num_initial_preds, $num_final_preds, $max_concrete_size, $num_used_preds, $failed_refinements, $last_frame \n')
    return t.substitute(locals())

if __name__ == '__main__':
    print('name, result, total time, z3 time, generalization time, recursive block time, propagation time, refinement time, minimization time, number of z3 calls, added diagrams per frame, greates number of literas per frame, # refinements, # initial preds, # final preds, max size, # used preds, # failed ref, # frames\n')
    directory = os.fsencode(sys.argv[1])    
    for file in os.listdir(directory):
        filename = os.fsdecode(file)
        if filename.endswith(".stdout"): 
            f = open(sys.argv[1] + '/' + filename)
            print(filename.replace('.stdout', '') + ', ' + do_filter(f.readlines()).replace('initial predicates', ''))
        else:
            continue
